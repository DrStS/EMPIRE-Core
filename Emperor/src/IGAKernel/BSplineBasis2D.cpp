...
BSplineBasis2D::BSplineBasis2D(int _ID = 0, int _pDegree = 0, int _noKnotsU = 0,
		double* _KnotVectorU = NULL, int _qDegree = 0, int _noKnotsV = 0,
		double* _KnotVectorV = NULL) :
		AbstractBSplineBasis2D(_ID) {

	// The NURBS basis functions in u-direction
	uBSplineBasis1D = new BSplineBasis1D(_ID, _pDegree, _noKnotsU,
			_KnotVectorU);

	// The NURBS basis functions in v-direction
	vBSplineBasis1D = new BSplineBasis1D(_ID, _qDegree, _noKnotsV,
			_KnotVectorV);
}
...
BSplineBasis2D& BSplineBasis2D::operator=(
		const BSplineBasis2D& _bSplineBasis2D) {
	if (this != &_bSplineBasis2D) {
		// Copy the NURBS basis functions in u-direction
		uBSplineBasis1D = new BSplineBasis1D(
				*(_bSplineBasis2D.uBSplineBasis1D));

		// Copy the NURBS basis functions in v-direction
		vBSplineBasis1D = new BSplineBasis1D(
				*(_bSplineBasis2D.vBSplineBasis1D));
	}
	return *this;
}

int BSplineBasis2D::indexDerivativeBasisFunction(int _derivDegree,
		int _uDerivIndex, int _vDerivIndex, int _basisIndex) {
	/*
	...
	if (_uDerivIndex + _vDerivIndex > _derivDegree) {
		...
		cout << "the " << _vDerivIndex
				<< "-th partial derivative w.r.t. v of the basis functions but "
				<< endl;
	...
	return (_derivDegree - _vDerivIndex) * (_derivDegree - _vDerivIndex + 1)
			* noBasisFcts / 2 + _uDerivIndex * noBasisFcts + _basisIndex;
}

void BSplineBasis2D::computeLocalBasisFunctions(double* _basisFcts,
		double _uPrm, int _KnotSpanIndexU, double _vPrm, int _KnotSpanIndexV) {
	/*
	...
	double* uBSplinebasis1DFcts =
			new double[uBSplineBasis1D->getPolynomialDegree() + 1];
	uBSplineBasis1D->computeLocalBasisFunctions(uBSplinebasis1DFcts, _uPrm,
			_KnotSpanIndexU);

	// Compute the local B-Spline basis functions in v-direction
	double* vBSplinebasis1DFcts =
			new double[vBSplineBasis1D->getPolynomialDegree() + 1];
	vBSplineBasis1D->computeLocalBasisFunctions(vBSplinebasis1DFcts, _vPrm,
			_KnotSpanIndexV);
	...
	for (int j = 0; j <= vBSplineBasis1D->getPolynomialDegree(); j++) {
		for (int i = 0; i <= uBSplineBasis1D->getPolynomialDegree(); i++) {
			...
			_basisFcts[counter] = uBSplinebasis1DFcts[i]
					* vBSplinebasis1DFcts[j];
...
void BSplineBasis2D::getBasisFunctionsIndex(int _KnotSpanIndexU,
		int _KnotSpanIndexV, int* _funcsIndex) {
	...
	// The numbering scheme for control points shows in IGAPatchSurface::computeCartesianCoordinates
	// They are different!!!
	for (int i = _KnotSpanIndexV - vBSplineBasis1D->getPolynomialDegree();
			i <= _KnotSpanIndexV; i++)
		for (int j = _KnotSpanIndexU - uBSplineBasis1D->getPolynomialDegree();
				j <= _KnotSpanIndexU; j++)
...
void BSplineBasis2D::computeLocalBasisFunctionsAndDerivativesInefficient(
		double** _basisFctsAndDerivs, int _maxMixDerivOrd, int _derivDegreeU,
		double _uPrm, int _KnotSpanIndexU, int _derivDegreeV, double _vPrm,
		int _KnotSpanIndexV) {
	/*
	...
	if (_maxMixDerivOrd > _derivDegreeU + _derivDegreeV) {
		...
		cout << endl;
		cout
				<< "Error in BSplineBasis2D::computeLocalBasisFunctionsAndDerivatives"
				<< endl;
		cout << "Requested order of partial derivatives du: " << _derivDegreeU
				<< " and dv: " << _derivDegreeV << endl;
		cout << "but requested maximal order of the mixed derivatives is dudv: "
				<< _maxMixDerivOrd << endl;
	...
	for (int i = 0; i <= _derivDegreeU; i++)
		uBSplinebasis1DFctsDerivs[i] =
				new double[uBSplineBasis1D->getPolynomialDegree() + 1];
	uBSplineBasis1D->computeLocalBasisFunctionsAndDerivativesInefficient(
			uBSplinebasis1DFctsDerivs, _derivDegreeU, _uPrm, _KnotSpanIndexU);
	...
	for (int i = 0; i <= _derivDegreeV; i++)
		vBSplinebasis1DFctsDerivs[i] =
				new double[vBSplineBasis1D->getPolynomialDegree() + 1];
	vBSplineBasis1D->computeLocalBasisFunctionsAndDerivativesInefficient(
			vBSplinebasis1DFctsDerivs, _derivDegreeV, _vPrm, _KnotSpanIndexV);
				...
				for (int l = 0; l <= _derivDegreeU; l++) {
					if (k + l <= _maxMixDerivOrd) {
						...
						_basisFctsAndDerivs[counterDegree][counterBasis] =
								uBSplinebasis1DFctsDerivs[l][i]
										* vBSplinebasis1DFctsDerivs[k][j];
...
void BSplineBasis2D::computeLocalBasisFunctionsAndDerivativesInefficient(
	...
	uBSplineBasis1D->computeLocalBasisFunctionsAndDerivatives(
			uBSplinebasis1DFctsDerivs, _derivDegreeU, _uPrm, _KnotSpanIndexU);

	// Compute the derivatives of the local B-Spline basis functions in v-direction
	double* vBSplinebasis1DFctsDerivs = new double[(_derivDegreeV + 1)
			* (vBSplineBasis1D->getPolynomialDegree() + 1)];
	vBSplineBasis1D->computeLocalBasisFunctionsAndDerivatives(
			vBSplinebasis1DFctsDerivs, _derivDegreeV, _vPrm, _KnotSpanIndexV);
		...
		for (int i = 0; i <= uBSplineBasis1D->getPolynomialDegree(); i++) {
			for (int k = 0; k <= _derivDegreeV; k++) {
				for (int l = 0; l <= _derivDegreeU; l++) {
					if (k + l <= _maxMixDerivOrd) {
						// Compute the functional value as a tensor product
						_basisFctsAndDerivs[counterDegree * noBasisFcts
								+ counterBasis] =
								uBSplinebasis1DFctsDerivs[l
										* (uBSplineBasis1D->getPolynomialDegree()
												+ 1) + i]
										* vBSplinebasis1DFctsDerivs[k
												* (vBSplineBasis1D->getPolynomialDegree()
														+ 1) + j];
...
void BSplineBasis2D::computeLocalBasisFunctionsAndDerivatives(
		double* _basisFctsAndDerivs, int _derivDegree, double _uPrm,
	...
	// Compute the derivatives of the local B-Spline basis functions in u-direction
	double* uBSplinebasis1DFctsDerivs = new double[(_derivDegree + 1)
			* (pDegree + 1)];
	uBSplineBasis1D->computeLocalBasisFunctionsAndDerivatives(
			uBSplinebasis1DFctsDerivs, _derivDegree, _uPrm, _KnotSpanIndexU);

	// Compute the derivatives of the local B-Spline basis functions in v-direction
	double* vBSplinebasis1DFctsDerivs = new double[(_derivDegree + 1)
			* (qDegree + 1)];
	vBSplineBasis1D->computeLocalBasisFunctionsAndDerivatives(
			vBSplinebasis1DFctsDerivs, _derivDegree, _vPrm, _KnotSpanIndexV);
	...
	for (int vBasis = 0; vBasis <= qDegree; vBasis++) {
		for (int uBasis = 0; uBasis <= pDegree; uBasis++) {
			for (int k = 0; k <= du; k++) {
				...
				for (int l = 0; l <= dd; l++) {
					// Compute the index of the functional value
					index = indexDerivativeBasisFunction(_derivDegree, k, l,
							counterBasis);

					// Compute the functional value in 2D as a tensor product of the basis functions in 1D
					_basisFctsAndDerivs[index] = uBSplinebasis1DFctsDerivs[k
							* (pDegree + 1) + uBasis]
							* vBSplinebasis1DFctsDerivs[l * (qDegree + 1)
									+ vBasis];
				}
			}
...
void BSplineBasis2D::printPolynomialDegrees() {
	...
	cout << "BSplineBasis2D::PDegree = "
			<< uBSplineBasis1D->getPolynomialDegree() << endl;
	cout << "BSplineBasis2D::QDegree = "
			<< vBSplineBasis1D->getPolynomialDegree() << endl;
...
void BSplineBasis2D::printNoKnots() {
	...
	cout << "---------------------------------------------" << endl;
	cout << "Debugging information in class BSplineBasis2D" << endl;
	cout << "BSplineBasis2D::uNoKnots = " << uBSplineBasis1D->getNoKnots()
			<< endl;
	cout << "BSplineBasis2D::vNoKnots = " << vBSplineBasis1D->getNoKnots()
			<< endl;
	cout << "_____________________________________________" << endl;
	cout << endl;
}
...
void BSplineBasis2D::printNoBasisFunctions() {
	...
	cout << "Debugging information in class BSplineBasis2D" << endl;
	cout << endl;
	cout << "BSplineBasis1D::uNoBasisFunctions = "
			<< uBSplineBasis1D->computeNoBasisFunctions() << endl;
	cout << "BSplineBasis1D::vNoBasisFunctions = "
			<< vBSplineBasis1D->computeNoBasisFunctions() << endl;
	cout << "_____________________________________________" << endl;
	cout << endl;
}
